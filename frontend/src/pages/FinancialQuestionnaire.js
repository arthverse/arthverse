...
export default function FinancialQuestionnaire({ token, onLogout }) {
  ...
  const defaultFormData = {
    ...
    q1_health_insurance: '',  // Personal health insurance
    q2_term_insurance: '',    // Term life insurance
    q3_itr_filing: '',        // ITR filing
    q4_credit_card: '',       // Credit card ownership
    q5_cc_balance: '',        // Revolving credit card balance
    q6_personal_loan: '',     // Personal loan for consumption
    q7_regular_investing: '', // Regular investing beyond FD
          ...
          {step === 5 && (
            <Card className="p-6 bg-white border border-slate-200 rounded-2xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-semibold text-slate-900">✈️ Free Flights</p>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[
                    { key: 'spend_bills_utilities', label: '📱 Bills & Utilities', icon: '📱' },
                    { key: 'spend_groceries', label: '🛒 Groceries & Quick Commerce', icon: '🛒' },
                    { key: 'spend_online_shopping', label: '🛍️ Online Shopping', icon: '🛍️' },
                    { key: 'spend_dining_food', label: '🍔 Dining & Food Delivery', icon: '🍔' },
                    { key: 'spend_upi_merchants', label: '📲 UPI Payments (Merchants)', icon: '📲' },
                    { key: 'spend_instore_shopping', label: '🏪 In-Store Shopping', icon: '🏪' },
                    { key: 'spend_flights_hotels', label: '✈️ Flights & Hotels', icon: '✈️' },
                    { key: 'spend_rent_payments', label: '🏠 Rent Payments', icon: '🏠' },
                    { key: 'spend_insurance', label: '🛡️ Insurance', icon: '🛡️' },
                    { key: 'spend_forex', label: '💱 Forex Spends', icon: '💱' },
                    { key: 'spend_fuel', label: '⛽ Fuel', icon: '⛽' },
                    { key: 'spend_jewellery_gold', label: '💎 Jewellery & Gold', icon: '💎' },
                    { key: 'spend_government_tax', label: '🏛️ Government/Tax Payments', icon: '🏛️' },
                    { key: 'spend_education', label: '📚 Education', icon: '📚' }
                  ].map(category => (
